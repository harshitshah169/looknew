...
const MansClothingPage = () => {
  const price399  = useFirestore('MansClothing-399');
  const price499 = useFirestore('MansClothing-499');
  const price599 = useFirestore('MansClothing-599');
  const price749 = useFirestore('MansClothing-749');
  const price999 = useFirestore('MansClothing-999');
  const price1199 = useFirestore('MansClothing-1199');
  const price1499 = useFirestore('MansClothing-1499');
  const price1749 = useFirestore('MansClothing-1749');
  const price1999 = useFirestore('MansClothing-1999');
  const price2000 = useFirestore('MansClothing-moreThan2000');
                ...
                <div class="container-individual-product" id="product-image-list">
                  ...
                  { price399 && price399.map(doc => (
                    <div style={{display: 'block', flexDirection: 'horizontal', marginTop: '20px'}}>
                      <img src={doc.url} key={doc.id} alt="Mans-cloths" className="product-type" id="individual-product-image"/>
                      <div>
...
export default MansClothingPage;